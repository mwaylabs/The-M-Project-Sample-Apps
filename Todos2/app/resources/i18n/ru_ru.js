M.I18N.ru_ru = {

    "tab_list": "Список задач",

    "tab_new": "Новая задача",

    "tab_settings": "Настройки",

    "edit": "Редактировать",

    "add": "Добавить",

    "save": "Сохранить",

    "back": "Назад",

    "title": "Название",

    "details": "Подробности",

    "due_date": "Дата выполнения",

    "error": "Ошибка",

    "title_req": "'Название' не заполнено.",

    "details_req": "'Подробности' не заполнены.",

    "due_date_req": "'Дата выполнения' не заполнена.",

    "due_date_invalid": "'Дата выполнения' некорректна.",

    "delete": "Удалить",

    "delete_": "Удалить?",

    "due_date_format": "dd/mm/yyyy HH:MM",

    "settings": "Настройки",

    "language": "Язык",

    "german": "Deutsch",

    "english": "English",

    "spanish": "Español",

    "portuguese": "Português",

    "chinese-simple": "简体中文",

    "chinese-traditional": "繁體中文",

    "russian": "Русский",

    "italian": "Italiano",

    "polish": "Polski",

    "dutch": "Nederlands",

    "slovakian": "Slovenský",

    "korean": "한국어",

    "swedish": "Svenska",
    
    "finnish": "Suomi",
    
    "days": "дней",

    "contact": "Контакты",

    "contactText": "За дополнительной информацией, касающейся этого приложения или 'The-M-Project', пожалуйста, обращайтесь в Panacoda GmbH.",

    "confirm": "Вы действительно хотите удалить эту задачу?",

    "ok": "Ok",

    "cancel": "Отменить",

    "error": "Ошибка"

}